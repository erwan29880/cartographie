...
class Afficher:
    ...
    def tracer_cercle(self, x, y, couleur = 'purple', rayon=6, remplissage=True):
        x1 = x - rayon
        x2 = x + rayon
        y1 = y - rayon 
        y2 = y + rayon 
        ...
        if self.qgis == True:
            for i in range(self.coordonnees.shape[0]):
                ...
                if i==0:  # --> afficher le point de départ
                    self.tracer_cercle(abs, ord, couleur='red', rayon=9)
                else:
                    self.tracer_cercle(abs, ord)
            ...
            for i in range(self.coordonnees.shape[0]):
                if i==0:  # --> afficher le point de départ
                    self.tracer_cercle(self.coordonnees.iloc[i, 0], self.coordonnees.iloc[i, 1], couleur='red', rayon=9)
                else:
                    self.tracer_cercle(self.coordonnees.iloc[i, 0], self.coordonnees.iloc[i, 1])
    ...
    def dessiner_chemin_optimise(self, df):
        ...
        if self.qgis == True:
            ...
            for i in range(df.shape[0]-1):
                ...
                if i==0:  # --> afficher le point de départ
                    self.tracer_cercle(abs1, ord1, couleur='red', rayon=9)
                else:
                    self.tracer_cercle(abs1, ord1)
            ...
            for i in range(df.shape[0]-1):
                ...
                if i==0:  # --> afficher le point de départ
                    self.tracer_cercle(x1,y1, couleur='red', rayon=9)
                else:
                    self.tracer_cercle(x1, y1)
